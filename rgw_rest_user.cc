...
void RGWOp_Quota_Set::execute()
{
  ...
  bool bucket_type = (quota_type == "bucket");
  bool bucket_empty = (bucket != "");
  bool set_all = quota_type.empty();
  bool set_bucket = set_all || bucket_type;
  bool set_user = set_all || (quota_type == "user");
  bool set_detail_bucket = bucket_type && bucket_empty;

  if (!(set_all || set_bucket || set_user || set_detail_bucket)) {
 ...
 #define QUOTA_INPUT_MAX_LEN 1024 
  if(set_detail_bucket){
    RGWQuotaInfo quotaes;
    http_ret = rgw_rest_get_json_input(store->ctx(), s, quotaes, QUOTA_INPUT_MAX_LEN, NULL);
    if(http_ret < 0) {
    ...
    http_ret = store->get_bucket_info(obj_ctx, s->bucket_tenant, bucket, bucket_info, NULL, &attrs);
    if(http_ret < 0){
       ldout(store->ctx(),1) << "could not get bucket info for bucket=" << bucket<< dendl;
       return;
    }
    quotaes.enabled= true;
    bucket_info.quota = quotaes;
    http_ret = store->put_bucket_instance_info(bucket_info, false, real_time(), &attrs);
    if(http_ret < 0){
      ldout(store->ctx(),1)<< "ERROR: failed writing bucket instance info: " <<bucket<< dendl;
      return;
    }
    return;
  }  
