...
int RGWPutMetadataObject_ObjStore_S3::get_params()
{
    if(s->has_bad_meta)
    	return -EINVAL;
    return 0;
...
void RGWPutMetadataObject_ObjStore_S3::send_response()
{
  if (op_ret)
     set_req_state_err(s, op_ret);
  dump_errno(s);
...
RGWOp *RGWHandler_REST_Obj_S3::op_post()
{
  ...
  if(s->info.args.exists("metadata"))
    return new RGWPutMetadataObject_ObjStore_S3;
